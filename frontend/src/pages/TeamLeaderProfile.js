// Team Leader Profile Page - Employee Profile + Hierarchy Tree (teams → employees)
...
const TeamLeaderProfile = () => {
  ...
  const fetchTeamLeaderData = useCallback(async () => {
    ...
    try {
      ...
      if (employeeResponse.data && employeeResponse.data.data) {
        ...
        // Fetch hierarchy data (teams and employees)
        try {
        ...
        setError('Team Leader profile not found.');
      }
    } catch (error) {
      ...
      if (error.response?.status === 404) {
        ...
        setError('Failed to load team leader profile. Please try again.');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Layout>
      <div className="max-w-4xl mx-auto p-6">
        <h1 className="text-3xl font-bold mb-6" style={{ color: 'var(--text-primary)' }}>Team Leader Profile</h1>
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h2 className="text-3xl font-bold mb-2" style={{ color: 'var(--text-primary)' }}>
                    {employee.name || 'Team Leader Name'}
                  </h2>
