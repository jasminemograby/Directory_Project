-- Add Requests Tables Migration
-- Creates tables for training, skill verification, self-learning, and extra attempt requests
...
-- Update extra_attempt_requests table to match new structure (if needed)
-- Add company_id if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'extra_attempt_requests' AND column_name = 'company_id'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'extra_attempt_requests' AND column_name = 'course_name'
    ) THEN
        ALTER TABLE extra_attempt_requests ADD COLUMN course_name VARCHAR(255);
    END IF;
    
    -- Add notes if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'extra_attempt_requests' AND column_name = 'notes'
    ) THEN
